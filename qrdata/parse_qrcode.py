...
from pyzbar.pyzbar import decode, ZBarSymbol#解析二维码的库
import zxing#解析二维码的库
# 解析方法一:借助pyzbar
def pyzbarParseQRCode(filePath, data_encoding):
    if os.path.isfile(filePath):# 判断是否是本地文件
        img = cv.imread(filePath)#读取二维码图片
        # texts = decode(img, symbols=[ZBarSymbol.QRCODE])#解码验证码图片
...
if __name__ == '__main__':
    ...
    pyzbarParseQRCode(filePath, 'ascii')
